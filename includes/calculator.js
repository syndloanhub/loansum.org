...
// Loansum service. This service encapsulates the loansum data model and
// all interactions with the loansum application service.

function LoansumService($http) {
  var service = this;
  
  console.log("enter LoansumService");

  // Bootstrap data and UI model from server file.
...
angular.module('Calculator', [ 'smart-table' ]).controller('CalculatorController', CalculatorController)
    .service('LoansumService', LoansumService).filter('unique', function() {
...
CalculatorController.$inject = [ '$scope', '$http', 'LoansumService' ];

function CalculatorController($scope, $http, LoansumService) {
  ...
  $scope.scrollContract = function() {
    ...
    displayedIndex = $scope.displayedContracts.indexOf($scope.uimodel[$scope.selectedLoanIndex].contracts[index]) + 1;
  ...
  $scope.scrollFacilityEvent = function() {
    ...
    displayedIndex = $scope.displayedFacilityEvents.indexOf($scope.uimodel[$scope.selectedLoanIndex].events[index]) + 3;
  ...
  $scope.removeEvent = function removeEvent(event) {
    index = $scope.uimodel[$scope.selectedLoanIndex].contracts[$scope.selectedContractIndex].events.indexOf(event);
    if (index !== -1) {
