    ...
    .controller(
        ...
        function($scope) {
          ...
          $scope.JSONcontractIndex = {
            "LIBOR" : "USD-LIBOR-3M",
            "PRIME" : "USD-LIBOR-3M" /*
                                       * update this when get info from John
                                       */
          };
          ...
          $scope.createTradeJSON = function createTradeJSON() {
            ...
            for (i in $scope.selectedLoan.contracts) { // or switch
              ...
              $scope.tradeJSON.product.contracts[i].paymentDate = $scope.selectedLoan.contracts[i].paymentDate.toJSON()
                  .slice(0, 10);
            ...
            xhr.onreadystatechange = function() {
              if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if ($scope.data.message) {
                  ...
                  ;
                  /*
                   * angular.forEach(data.cashFlows,){} for (i=0, i <
                   * data.cashFlows.length, i++){
                   * switch(data.cashFlows[i].annotation.type){ case
                   * "CostOfFunded":
                   * $scope.selectedTrade.tradeProceeds.costOfFunded =
                   * data.cashFlows[i].cashFlow.forecastValue; break; case
                   * "benefitOfUnfunded":
